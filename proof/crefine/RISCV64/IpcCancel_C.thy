...
lemma cancelIPC_ccorres_helper2:
  "ccorres dc xfdc (invs' and (\<lambda>s. sym_refs (state_refs_of' s)) and
         st_tcb_at' (\<lambda>st. (isBlockedOnSend st \<or> isBlockedOnReceive st)
                            \<and> replyObject st = maybeReply) thread
        and ko_at' reply' reply)
        {s. reply_' s = Ptr reply}
        []
        (case maybeReply of None \<Rightarrow> return () | Some reply \<Rightarrow> replyUnlink reply thread)
...
lemma cancelIPC_ccorres1_helper_x_1:
  "\<And>a b c. (case Structures_H.thread_state.BlockedOnReceive a b c of
                Structures_H.thread_state.BlockedOnReceive a _ _ \<Rightarrow> Just a
                | Structures_H.thread_state.BlockedOnSend a _ _ _ _  \<Rightarrow> Just a
                | _ \<Rightarrow> Nothing) = Just a"
  by (simp_all)

lemma cancelIPC_ccorres1_helper_x_2:
